...
import { addDepense } from '../../services/budgetApi';
...
export default function AddDepenseScreen() {
  const router = useRouter();
  const [chantiers, setChantiers] = useState([]);
  const [loadingChantiers, setLoadingChantiers] = useState(true);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ chantier_id: null, description: '', montant: '', categorie: 'materiel', date: new Date().toISOString().split('T')[0] });
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await addDepense({ chantier_id: form.chantier_id, description: form.description, montant: parseFloat(form.montant), categorie: form.categorie, date: form.date });
      Alert.alert('Succès', 'Dépense ajoutée', [{ text: 'OK', onPress: () => router.back() }]);
    } catch (err) {
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50" contentContainerStyle={{ padding: 16 }}>
      <View className="bg-red-600 rounded-2xl p-5 mb-5">
        <Text className="text-white text-xl font-bold">➕ Nouvelle dépense</Text>
      </View>

      <Text className="text-sm font-semibold text-gray-600 mb-2">Chantier</Text>
      <View className="flex-row flex-wrap mb-4">
        {chantiers.map((c) => (
          <TouchableOpacity key={c.id} className={`px-3 py-2 rounded-full border mr-2 mb-2 ${form.chantier_id === c.id ? 'bg-red-600 border-red-600' : 'border-gray-200 bg-white'}`} onPress={() => setForm({ ...form, chantier_id: c.id })}>
      ...
      <TextInput className="border border-gray-200 rounded-xl px-4 py-3 bg-white mb-4" placeholder="ex: Achat ciment" value={form.description} onChangeText={(v) => setForm({ ...form, description: v })} />

      <Text className="text-sm font-semibold text-gray-600 mb-1">Montant (Ar)</Text>
      <TextInput className="border border-gray-200 rounded-xl px-4 py-3 bg-white mb-4" placeholder="ex: 500000" keyboardType="numeric" value={form.montant} onChangeText={(v) => setForm({ ...form, montant: v })} />
      ...
      <Text className="text-sm font-semibold text-gray-600 mb-2">Catégorie</Text>
      <View className="flex-row flex-wrap mb-6">
        {CATS.map((c) => (
          <TouchableOpacity key={c} className={`px-3 py-2 rounded-full border mr-2 mb-2 ${form.categorie === c ? 'bg-red-600 border-red-600' : 'border-gray-200 bg-white'}`} onPress={() => setForm({ ...form, categorie: c })}>
            <Text className={`text-xs font-semibold ${form.categorie === c ? 'text-white' : 'text-gray-600'}`}>{CATS_LABEL[c]}</Text>
          </TouchableOpacity>
        ))}
      </View>

      <TouchableOpacity className={`py-4 rounded-xl items-center mb-4 ${saving ? 'bg-red-300' : 'bg-red-600'}`} onPress={handleSubmit} disabled={saving}>
        <Text className="text-white font-bold">{saving ? 'Ajout…' : 'Ajouter la dépense'}</Text>
      </TouchableOpacity>
